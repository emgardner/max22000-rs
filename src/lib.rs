...
mod tests {
    ...
    use embedded_hal::spi::{
        ErrorKind, ErrorType, Operation as SpiTransactionOperation, SpiDevice,
    };
    ...
    impl FakeSpi {
        fn with_response<const N: usize>(response: [u8; N]) -> Self {
            Self {
                responses: VecDeque::from([response.to_vec()]),
                writes: Vec::new(),
            }
        }
    }
    ...
    impl SpiDevice<u8> for FakeSpi {
        fn transaction(
            &mut self,
            operations: &mut [SpiTransactionOperation<'_, u8>],
        ) -> Result<(), Self::Error> {
            for operation in operations {
                match operation {
                    SpiTransactionOperation::Read(read) => {
                        if let Some(response) = self.responses.pop_front() {
                            read.copy_from_slice(&response);
                        }
                    }
                    SpiTransactionOperation::Write(write) => {
                        self.writes.push(write.to_vec());
                    }
                    SpiTransactionOperation::Transfer(read, write) => {
                        self.writes.push(write.to_vec());
                        if let Some(response) = self.responses.pop_front() {
                            read.copy_from_slice(&response);
                        }
                    }
                    SpiTransactionOperation::TransferInPlace(buffer) => {
                        self.writes.push(buffer.to_vec());
                        if let Some(response) = self.responses.pop_front() {
                            buffer.copy_from_slice(&response);
                        }
                    }
                    SpiTransactionOperation::DelayNs(_) => {}
                }
            }
    ...
    #[test]
    fn operation_bits_match_datasheet_examples() {
        assert_eq!(Operation::Write(Registers::GenChannel).as_u8(), 0x06);
        assert_eq!(Operation::Read(Registers::GenConfig).as_u8(), 0x05);
    }

    #[test]
    fn read_register_uses_four_byte_transfer_without_crc() {
        let spi = FakeSpi::with_response([0x05, 0x10, 0x00, 0x00]);
        let mut max = Max22000::new(spi);

        let payload = max.read_register(Registers::GenConfig).unwrap();
        let spi = max.release();

        assert_eq!(payload, [0x10, 0x00, 0x00]);
        assert_eq!(spi.writes, [vec![0x05, 0x00, 0x00, 0x00]]);
    }

    #[test]
    fn write_register_uses_four_byte_write_without_crc() {
        let spi = FakeSpi::default();
        let mut max = Max22000::new(spi);

        max.write_register(Registers::GenConfig, [0x10, 0x00, 0x00])
            .unwrap();
        let spi = max.release();

        assert_eq!(spi.writes, [vec![0x04, 0x10, 0x00, 0x00]]);
    }

    #[test]
    fn read_register_checks_crc_when_enabled() {
        let mut response = [0x05, 0x10, 0x00, 0x00, 0x00];
        response[4] = Max22000::<FakeSpi>::crc_buffer(&response[..4]);
        let spi = FakeSpi::with_response(response);
        let mut max = Max22000::new(spi);
        max.set_crc(true);

        let payload = max.read_register(Registers::GenConfig).unwrap();
        let spi = max.release();
        let mut request = vec![0x05, 0x00, 0x00, 0x00, 0x00];
        request[4] = Max22000::<FakeSpi>::crc_buffer(&request[..4]);

        assert_eq!(payload, [0x10, 0x00, 0x00]);
        assert_eq!(spi.writes, [request]);
    }

    #[test]
    fn write_register_appends_crc_when_enabled() {
        let spi = FakeSpi::default();
        let mut max = Max22000::new(spi);
        max.set_crc(true);
        ...
        max.write_register(Registers::GenConfig, [0x10, 0x00, 0x00])
            .unwrap();
        let spi = max.release();
        let mut request = vec![0x04, 0x10, 0x00, 0x00, 0x00];
        request[4] = Max22000::<FakeSpi>::crc_buffer(&request[..4]);
    ...
    #[test]
    fn read_register_rejects_bad_crc() {
        let spi = FakeSpi::with_response([0x05, 0x10, 0x00, 0x00, 0x00]);
        let mut max = Max22000::new(spi);
        max.set_crc(true);

        assert!(matches!(
            max.read_register(Registers::GenConfig),
            Err(MaxError::CrcError)
        ));
    }
}
